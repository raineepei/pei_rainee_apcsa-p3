...
public class ElevensBoard extends Board {
	...
	public boolean isLegal(List<Integer> selectedCards) {
		...
		if (selectedCards.size() == 2 && containsPairSum11(selectedCards) 
			|| selectedCards.size() == 3 && containsJQK(selectedCards)) return true;
		else return false;
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
		...
		for (int i: selectedCards)
		{
			...
			{
				if (cardAt(i).pointValue() + cardAt(j).pointValue() == 11) return true;
			}
		}
		return false;
	}
	...
	private boolean containsJQK(List<Integer> selectedCards) {
		...
		for (int i: selectedCards)
		{
			if (cardAt(i).rank().equals("jack")) containsJ = true;
			else if (cardAt(i).rank().equals("queen")) containsQ = true;
			else if (cardAt(i).rank().equals("king")) containsK = true;
		}
